<?php
   ...
   //if there is active session, display the end session link
   if($_SESSION['session'] == 'active')
   {
      $t->setVariable("session_button","<a href='resultScreen.php?session=end'>End session</a>"); 
      $t->addBlock("session_block");
      //display show wines names button if the list exists
   
      $t->setVariable("wine_button","<a href='winelist.php'>Wine list</a>");
      ...
      if($result->rowCount() > 0)
      {
            ...
            foreach($result2 as $row2)
	    {
	       ...
	       $varietylist .= $row2["variety"];
            ...
            //add wine name to wine list
            if($_SESSION['session'] == "active")
            {  
               //ignores duplicates
               if(!( in_array($row["wine_name"],$_SESSION['wines'])))
               {
                  array_push($_SESSION['wines'],$row["wine_name"]);
               }
            }
